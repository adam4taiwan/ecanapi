...
namespace Ecanapi.Controllers
{
    ...
    public class NineStarController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetDaily(string? lineUserId = null)
        {
            ...
            // 五對組合分析（主位×勳位，查33則KB，fallback五行計算）
            var combinationRules = await _context.NineStarCombinationRules.ToListAsync();
            var combinations = new[]
            {
                (pair: "命×流年", starA: natalStar,  starB: yearStar),
                (pair: "運×流年", starA: currentYun, starB: yearStar),
                (pair: "流年×流月", starA: yearStar,  starB: monthStar),
                (pair: "流月×流日", starA: monthStar, starB: dayStar),
                (pair: "流日×流時", starA: dayStar,   starB: hourStar),
            }.Select(c =>
            {
                // 先查命名規則KB（主位+勳位精確匹配）
                var named = combinationRules
                    .FirstOrDefault(r => r.StarA == c.starA && r.StarB == c.starB &&
                ...
                {
                    var (relation, fVerdict, note) = Ecanapi.Services.NineStarCalcHelper.CalcFiveElementCombination(c.starA, c.starB);
                    title = relation;
                    verdict = fVerdict;
                    description = note;
                }
                ...
                {
                    pair = c.pair,
                    starA = new { number = c.starA, name = StarNames[c.starA] },
                    starB = new { number = c.starB, name = StarNames[c.starB] },
                    title,
                    verdict,
                    modified,
                    description
            ...
            // 整體評語（以命×流年為主軸，流年×流月為輔）
            string mainVerdict = combinations[0].modified;
            string overallVerdict = $"本命星目前{yunLabel}，今年{combinations[0].modified}，本月{combinations[2].modified}，今日{combinations[3].modified}。";
