...
namespace Ecanapi.Services
{
    public class AstrologyService : IAstrologyService
    {
        ...
        private void DetermineAllAuxiliaryAndMinorStars(AstrologyCalculationContext context)
        {
            ...
            if (xunKongPos1 != 0) context.BadStars[xunKongPos1] = (string.IsNullOrEmpty(context.BadStars[xunKongPos1]) ? "" : context.BadStars[xunKongPos1].TrimEnd() + " ") + "旬";
            if (xunKongPos2 != 0) context.BadStars[xunKongPos2] = (string.IsNullOrEmpty(context.BadStars[xunKongPos2]) ? "" : context.BadStars[xunKongPos2].TrimEnd() + " ") + "旬";
            // 5. 破碎 (論年支)
        ...
        private void DetermineDecadeLuckCycles(AstrologyCalculationContext context)
        {
            ...
            // 陣列+i=兄弟方向(逆數用)，-i=父母方向(順數用)
            // 陽男陰女順(父母方向isPlus=false)，陰男陽女逆(兄弟方向isPlus=true)
